...
public class RegisterConfig {
     ...
     * @return 配置文件管理器
     */
    public static ConfigManager registerConfig(JavaPlugin plugin, String filePath, ConfigManager.CreateType createType, Class<?> clazz) {
        ConfigManager configManager = new ConfigManager(plugin, filePath, createType);
        configManager.loadConfig(clazz);
        return configManager;
    }
     ...
     * @return 配置文件管理器
     */
    public static ConfigManager registerConfig(JavaPlugin plugin, String filePath, ConfigManager.CreateType createType, boolean autoUpdate, Class<?> clazz) {
        ConfigManager configManager = new ConfigManager(plugin, filePath, createType, autoUpdate);
        configManager.loadConfig(clazz);
        return configManager;
    }
     ...
     * @param plugin     插件
     * @param filePath   文件路径
     * @param createType 如果文件不存在是否创建（如果为否则从插件路径里面释放）
     * @return 配置文件管理器
     */
    public static ConfigManager registerConfig(JavaPlugin plugin, String filePath, ConfigManager.CreateType createType) {
        return new ConfigManager(plugin, filePath, createType);
    }
     ...
     * @return 配置文件管理器
     */
    public static ConfigManager registerConfig(JavaPlugin plugin, String filePath, ConfigManager.CreateType createType, boolean autoUpdate) {
        return new ConfigManager(plugin, filePath, createType, autoUpdate);
    }
