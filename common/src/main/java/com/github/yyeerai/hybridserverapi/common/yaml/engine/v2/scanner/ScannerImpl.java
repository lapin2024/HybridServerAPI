...
import com.github.yyeerai.hybridserverapi.common.yaml.engine.v2.exceptions.ScannerException;
import com.github.yyeerai.hybridserverapi.common.yaml.engine.v2.exceptions.YamlEngineException;
import com.github.yyeerai.hybridserverapi.common.yaml.engine.v2.scanner.ScannerImpl.Chomping.Indicator;
        ...
        while (!found) {
            ...
            if (reader.peek() == '#') {
                ...
                if (columnBeforeComment != 0 && !(lastToken != null
                        && lastToken.getTokenId() == Token.ID.BlockEntry)) {
    ...
    private String scanFlowScalarNonSpaces(boolean doubleQuoted, Optional<Mark> startMark) {
        ...
        while (true) {
            ...
            if (!doubleQuoted && c == '\'' && reader.peek(1) == '\'') {
                ...
                reader.forward();
                c = reader.peek();
                if (!Character.isSupplementaryCodePoint(c) && CharConstants.ESCAPE_REPLACEMENTS.containsKey(
                    ...
                    reader.forward();
                } else if (!Character.isSupplementaryCodePoint(c) && CharConstants.ESCAPE_CODES.containsKey(
                        Character.valueOf((char) c))) {
                    // The character is a multi-digit escape sequence, with
                    // length defined by the value in the ESCAPE_CODES map.
                    length = CharConstants.ESCAPE_CODES.get(Character.valueOf((char) c)).intValue();
                    reader.forward();
